...
function App() {
  ...
  const [players, setPlayers] = useState<Player[]>(() => {
    ...
    let initialPlayers: Player[] = storedPlayers ? JSON.parse(storedPlayers) : [];
    
    // Migration: If we have legacy positions data, merge it into players
    if (storedPositions) {
      try {
        const positions: Position[] = JSON.parse(storedPositions);
        const positionMap = new Map<string, string>(); // playerId -> positionId
        
        positions.forEach(p => {
          if (p.playerId) {
            positionMap.set(p.playerId, p.id);
          }
        });

        initialPlayers = initialPlayers.map(p => ({
          ...p,
          positionId: positionMap.get(p.id) || null
        }));
        
        // Clear legacy data
        localStorage.removeItem('positions');
      } catch (e) {
        console.error("Failed to migrate positions", e);
      }
    }
